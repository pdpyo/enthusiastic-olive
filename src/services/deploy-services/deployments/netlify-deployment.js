...
module.exports = {
    deploy: async function (project, user, buildLogger) {
        ...
        await netlifyService.updateProjectFromNetlifySiteOrDeploy(netlifySite, project);
    ...
    handleProjectUpdated: async function(project, projectPartialUpdate, user) {
        // For v2 projects where the Netlify UI is not accessible to the users, make sure certain settings get synced with Neylify.
        if (project.wizard?.deployment?.settings?.sharedUser) {
            const { siteId, netlifyAccessToken } = await getNetlifySiteIdAndToken(project, user);
            // If custom domain has been 'PATCH'ed, update the Netlify site config accordingly
            if (projectPartialUpdate.customDomain !== undefined) {
                // If the user clears the custom domain, Netlify needs to reset to the stackbit.app domain.
                if (projectPartialUpdate.customDomain === null) {
                    const netlifySite = await netlifyService.getSite(siteId, netlifyAccessToken);
                    await netlifyService.updateSite(siteId, { custom_domain: `${netlifySite.name}.${config.netlify.shared.domain}` }, netlifyAccessToken);
                } else {
                    await netlifyService.updateSite(siteId, { custom_domain: projectPartialUpdate.customDomain }, netlifyAccessToken);
                }
            }
            // If environment variables are included with in the change. Sync those with building settings for the Netlify site.
            if (projectPartialUpdate['deploymentData.container.env']) {
                await netlifyService.setEnvironmentVariables(siteId, netlifyAccessToken, project.deploymentData.container.env);
            }
        }
        return project;
    },
    verifyCustomDomain: async function(project, user) {
        // If there is a custom domain, call the /ssl endpoint on Netlify to ensure everything is complete
        if (project.customDomain && project.defaultDomain) {
            const { siteId, netlifyAccessToken } = await getNetlifySiteIdAndToken(project, user);
            await netlifyService.updateSiteSslCert(siteId, netlifyAccessToken);
        }
    },
};
