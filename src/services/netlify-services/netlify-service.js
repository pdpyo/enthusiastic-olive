...
function createSite(project, {isAnon, isShared, userId}, netlifyPublicKey, netlifyToken, buildLogger, retry = 0, retryName = null) {
    ...
    if (isShared) {
        ...
        options.custom_domain = project.customDomain || `${options.name}.${config.netlify.shared.domain}`;
    ...
    if (_.get(themeConfig, 'import')) {
        ...
        if (projectUtils.isV2Supported(project)) {
            buildLogger.debug('Netlify: Adding build v2 KEYs to repo settings');
    ...
    if (project.wizard?.deployment?.settings?.sharedUser) {
        // In v2, the user does not have direct access to Netlify. Any environment variables they 
        // set should also be available during build on Netlify.
        if (project.container?.env) {
            options.build_settings = {
                env: project.container.env
            };
        }
    }
...
function updateSite(siteId, opts, netlifyToken) {
    return netlifyAPI(`/sites/${siteId}`, 'patch', opts, netlifyToken);
}

function updateSiteSslCert(siteId, netlifyToken) {
    return netlifyAPI(`/sites/${siteId}/ssl`, 'post', null, netlifyToken);
}
...
function updateProjectFromNetlifySiteOrDeploy(netlifySiteOrDeploy, project) {
    const deployAdminUrl = netlifySiteOrDeploy.admin_url;
    const projectAdminUrl = project.deploymentData?.netlify?.url;
    const adminUrlChanged = deployAdminUrl && projectAdminUrl !== deployAdminUrl;

    const deploySiteUrl = netlifySiteOrDeploy.ssl_url;
    const projectSiteUrl = project.siteUrl;
    const siteUrlChanged = deploySiteUrl && projectSiteUrl !== deploySiteUrl;

    const deployCustomDomain = netlifySiteOrDeploy.custom_domain?.endsWith(config.netlify.shared.domain) ? null : netlifySiteOrDeploy.custom_domain;
    const projectCustomDomain = project.customDomain;
    const customDomainChanged = deployCustomDomain && projectCustomDomain !== deployCustomDomain;

    const deployDefultDomain = netlifySiteOrDeploy.default_domain;
    const projectDefaultDomain = project.defaultDomain;
    const defaultDomainChanged = deployDefultDomain && projectDefaultDomain !== deployDefultDomain;

    if (customDomainChanged) {
        project.customDomain = deployCustomDomain ?? null;
    }

    if (defaultDomainChanged) {
        project.defaultDomain = deployDefultDomain ?? null;
    }
    ...
    if (adminUrlChanged || siteUrlChanged) {
        ...
        const allowedHosts = project.allowedHosts || [];

        project.deploymentData.netlify = project.deploymentData.netlify || {};
        project.deploymentData.netlify.url = adminUrl;
        project.siteUrl = siteUrl;
        project.deploymentData.netlify.nameChanged = netlifySiteOrDeploy.name;
        project.metrics.didChangeNetlifyName = true;
        project.allowedHosts = [...(new Set([host, ...allowedHosts]))];

        const cms = project.wizard.cms.id;
        if (cms === 'netlifycms' && project.deploymentData.netlifycms) {
            project.deploymentData.netlifycms.url = `${siteUrl}/admin`;
        }
    }

    if (adminUrlChanged || siteUrlChanged || customDomainChanged || defaultDomainChanged) {
